...
def create_draft(
    width: int = Field(default=1920, ge=1, description="视频宽度分辨率，单位像素，如1920、1080"),
    height: int = Field(default=1080, ge=1, description="视频高度分辨率，单位像素，如1080、1920")
) -> dict:
    """创建剪映草稿"""
...
@mcp.tool(title="添加视频", description="向剪映草稿批量添加视频轨道和视频片段。时间单位为微秒(1秒=1000000微秒)")
def add_videos(
    draft_url: str = Field(..., description="草稿URL，由create_draft返回"),
    video_infos: str = Field(..., description="""视频JSON字符串数组，格式：[{"video_url":"https://example.com/video.mp4","width":1920,"height":1080,"start":0,"end":6000000,"duration":6000000,"mask":"","transition":"","transition_duration":500000,"volume":1}]
...
- volume: number, (可选)音量大小，范围0-10，默认1"""),
    alpha: float = Field(default=1.0, ge=0.0, le=1.0, description="全局透明度，值范围0.0-1.0"),
    scale_x: float = Field(default=1.0, description="X轴缩放比例，1.0为原始大小"),
    scale_y: float = Field(default=1.0, description="Y轴缩放比例，1.0为原始大小"),
    transform_x: float = Field(default=0.0, description="X轴位置偏移，单位像素，正值向右"),
    transform_y: float = Field(default=0.0, description="Y轴位置偏移，单位像素，正值向下"),
    scene_timelines: Optional[str] = Field(default="[]", description="""分镜时间线JSON数组，用于自动变速，格式：[{"start":0,"end":6000000,"curve_speed":"linear"}]
- start: number, 分镜开始时间
- end: number, 分镜结束时间
...
@mcp.tool(title="添加音频", description="向剪映草稿批量添加音频轨道和音频片段。时间单位为微秒(1秒=1000000微秒)")
def add_audios(
    draft_url: str = Field(..., description="草稿URL，由create_draft返回"),
    audio_infos: str = Field(..., description="""音频JSON字符串数组，格式：[{"audio_url":"https://example.com/audio.mp3","duration":12000000,"start":0,"end":12000000,"audio_effect":"教堂"}]
...
@mcp.tool(title="添加图片", description="向剪映草稿批量添加图片轨道和图片片段。时间单位为毫秒(1秒=1000毫秒)")
def add_images(
    draft_url: str = Field(..., description="草稿URL，由create_draft返回"),
    image_infos: str = Field(..., description="""图片JSON字符串数组，格式：[{"image_url":"https://example.com/img.png","width":1920,"height":1080,"start":0,"end":5000,"duration":5000,"animation":"淡入淡出","transition":"溶解","transition_duration":500}]
...
@mcp.tool(title="添加贴纸", description="向剪映草稿添加贴纸轨道和贴纸片段。时间单位为微秒(1秒=1000000微秒)，批处理时并发要设置成1")
def add_sticker(
    draft_url: str = Field(..., description="草稿URL，由create_draft返回"),
    sticker_id: str = Field(..., description="贴纸唯一标识ID，可通过search_sticker搜索获取"),
    start: int = Field(default=0, ge=0, description="贴纸在时间轴上的开始时间，单位微秒"),
    end: int = Field(default=5000000, ge=0, description="贴纸在时间轴上的结束时间，单位微秒"),
    scale: float = Field(default=1.0, description="贴纸缩放比例，1.0为原始大小"),
    transform_x: float = Field(default=0.0, description="X轴位置偏移，单位像素，正值向右"),
    transform_y: float = Field(default=0.0, description="Y轴位置偏移，单位像素，正值向下"),
) -> dict:
    """向剪映草稿添加贴纸"""
...
def add_keyframes(
    draft_url: str = Field(..., description="草稿URL，由create_draft返回"),
    keyframes: str = Field(..., description="""关键帧JSON字符串数组，格式：[{"offset":5000000,"property":"KFTypePositionX","segment_id":"abc123","value":0.5}]
...
@mcp.tool(title="添加字幕", description="向剪映草稿批量添加字幕轨道和字幕片段。caps参数内的所有时间单位为微秒(1秒=1000000微秒)")
def add_captions(
    draft_url: str = Field(..., description="草稿URL，由create_draft返回"),
    captions: str = Field(..., description="""字幕JSON字符串数组，格式：[{"start":0,"end":10000000,"text":"字幕文本","keyword":"关键词","keyword_color":"#ff0000","keyword_font_size":18}]
...
- keyword_font_size: number, (可选)关键词字体大小"""),
    text_color: str = Field(default="#ffffff", description="文字颜色，十六进制格式如#ffffff"),
    border_color: Optional[str] = Field(default=None, description="边框颜色，十六进制格式如#000000"),
    alignment: int = Field(default=1, ge=0, le=5, description="对齐方式: 0-左对齐, 1-居中, 2-右对齐, 3-左下, 4-居中, 5-右下"),
    alpha: float = Field(default=1.0, ge=0.0, le=1.0, description="透明度，值范围0.0-1.0"),
    font: Optional[str] = Field(default=None, description="字体名称，如'思源黑体'、'抖音体'等"),
    font_size: int = Field(default=15, ge=1, description="字体大小，数值越大字越大"),
    letter_spacing: Optional[float] = Field(default=None, description="字间距，数值越大字间距越大"),
    line_spacing: Optional[float] = Field(default=None, description="行间距倍数，1.0为原始行距"),
    scale_x: float = Field(default=1.0, description="X轴缩放比例，1.0为原始大小"),
    scale_y: float = Field(default=1.0, description="Y轴缩放比例，1.0为原始大小"),
    transform_x: float = Field(default=0.0, description="X轴偏移量，单位像素，正值向右"),
    transform_y: float = Field(default=0.0, description="Y轴偏移量，单位像素，正值向下"),
    style_text: int = Field(default=0, ge=0, le=1, description="是否使用样式文本，0-不使用，1-使用"),
    ...
    shadow_info: Optional[Dict[str, Any]] = Field(default=None, description="""阴影参数对象，当has_shadow为true时生效，结构：
- shadow_alpha: number, 阴影不透明度，范围0-1
- shadow_color: string, 阴影颜色，十六进制如"#000000"
- shadow_diffuse: number, 阴影扩散程度，范围0-100
- shadow_distance: number, 阴影距离，范围0-100
- shadow_angle: number, 阴影角度，范围-180到180"""),
) -> dict:
    """向剪映草稿批量添加字幕"""
...
@mcp.tool(title="添加特效", description="向剪映草稿添加特效轨道和特效片段。时间单位为微秒(1秒=1000000微秒)")
def add_effects(
    draft_url: str = Field(..., description="草稿URL，由create_draft返回"),
    effect_infos: str = Field(..., description="""特效JSON字符串数组，格式：[{"effect_title":"金粉闪闪","start":0,"end":5000000}]
...
@mcp.tool(title="添加遮罩", description="向剪映草稿中的素材添加蒙版遮罩")
def add_masks(
    draft_url: str = Field(..., description="草稿URL，由create_draft返回"),
    segment_ids: List[str] = Field(..., description="素材片段ID列表，从add_videos或add_images的返回值获取"),
    name: str = Field(default="线性", description="遮罩类型名称，可选值：线性、镜面、圆形、矩形、爱心、星形"),
    X: int = Field(default=0, description="遮罩中心X坐标，单位像素"),
    Y: int = Field(default=0, description="遮罩中心Y坐标，单位像素"),
    width: int = Field(default=512, description="遮罩宽度，单位像素"),
    height: int = Field(default=512, description="遮罩高度，单位像素"),
    feather: int = Field(default=0, ge=0, le=100, description="羽化程度，范围0-100"),
    rotation: int = Field(default=0, ge=0, le=360, description="旋转角度，范围0-360度"),
    invert: bool = Field(default=False, description="是否反转遮罩，true为反转"),
    roundCorner: int = Field(default=0, ge=0, le=100, description="矩形圆角半径，范围0-100"),
) -> dict:
    """向剪映草稿添加遮罩"""
...
def add_text_style(
    text: str = Field(..., description="完整文本内容，用于匹配和替换"),
    keyword: str = Field(default="", description="需要高亮的关键词，多个用|分隔，如\"快乐|顶级思维\""),
    font_size: int = Field(default=24, ge=1, description="普通文本的字体大小"),
    keyword_color: str = Field(default="#ff7100", description="关键词文本颜色，十六进制格式如#ff7100"),
    keyword_font_size: int = Field(default=24, ge=1, description="关键词字体大小")
) -> dict:
    """为文本创建富文本样式"""
...
def easy_create_material(
    draft_url: str = Field(..., description="草稿URL，由create_draft返回"),
    audio_url: str = Field(..., description="音频文件URL地址，不能为空"),
    text: Optional[str] = Field(default=None, description="要添加的文字内容"),
    img_url: Optional[str] = Field(default=None, description="图片文件URL地址"),
    video_url: Optional[str] = Field(default=None, description="视频文件URL地址"),
    text_color: str = Field(default="#ffffff", description="文字颜色，十六进制格式如#ffffff"),
    font_size: int = Field(default=15, ge=1, description="字体大小"),
    text_transform_y: float = Field(default=0.0, description="文字Y轴位置偏移，单位像素，正值向下")
) -> dict:
    """快速创建素材轨道"""
...
def get_text_animations(
    mode: int = Field(default=0, ge=0, le=2, description="动画模式：0-入场动画, 1-出场动画, 2-循环动画"),
    type: int = Field(default=0, ge=0, le=1, description="筛选类型：0-全部")
) -> dict:
    """获取文字出入场动画"""
...
def get_draft(
    draft_id: str = Field(..., description="草稿ID，从draft_url中提取，如URL为 https://xxx/get_draft?draft_id=abc123，则draft_id为abc123")
) -> dict:
    """获取草稿文件列表"""
...
def gen_video(
    draft_url: str = Field(..., description="草稿URL，由create_draft返回"),
    apiKey: str = Field(default="", description="(可选)API密钥，用于调用外部视频生成服务")
) -> dict:
    """生成视频 - 根据草稿URL导出视频"""
...
def get_audio_duration(
    mp3_url: str = Field(..., description="音频文件URL地址")
) -> dict:
    """获取音频文件时长"""
...
def timelines(
    duration: int = Field(..., ge=0, description="总时长，单位微秒，如10000000代表10秒"),
    num: int = Field(default=1, ge=1, description="分割数量，即时间线上的个数"),
    start: int = Field(default=0, ge=0, description="开始偏移量，单位微秒，默认0"),
    type: int = Field(default=0, ge=0, le=1, description="分割方式：0-平均分割，1-随机分割")
) -> dict:
    """计算时间线"""
...
def audio_timelines(
    links: str = Field(..., description="""音频URL JSON字符串数组，格式：["https://example.com/audio1.mp3","https://example.com/audio2.mp3"]
- 内部会自动获取每个音频的时长
...
def audio_infos(
    mp3_urls: str = Field(..., description="""音频URL JSON字符串数组，格式：["https://example.com/audio1.mp3","https://example.com/audio2.mp3"]"""),
    timelines_str: str = Field(..., description="""时间线JSON字符串数组，格式：[{"start":0,"end":284891428},{"start":284891428,"end":579578774}]
- start: number, 开始时间，单位微秒
- end: number, 结束时间，单位微秒"""),
    audio_effect: str = Field(default="", description="音频特效名称，如\"教堂\"、\"混响\"等"),
    volume: float = Field(default=1.0, ge=0.0, le=1.0, description="音量大小，范围0.0-1.0")
) -> dict:
    """根据音频URL和时间线生成音频信息"""
...
@mcp.tool(title="图片信息", description="根据图片URL列表和时间线生成图片信息数组，用于添加到视频轨道")
def imgs_infos(
    imgs: str = Field(..., description="""图片URL JSON字符串数组，格式：["https://example.com/img1.png","https://example.com/img2.png"]"""),
    timelines_str: str = Field(..., description="""时间线JSON字符串数组，格式：[{"start":0,"end":5000},{"start":5000,"end":10000}]
- start: number, 开始时间，单位毫秒
- end: number, 结束时间，单位毫秒"""),
    height: int = Field(default=1080, ge=1, description="图片高度，单位像素"),
    width: int = Field(default=1920, ge=1, description="图片宽度，单位像素"),
    in_animation: str = Field(default="", description="入场动画名称，如\"渐显出现\"、\"缩放入场\""),
    in_animation_duration: float = Field(default=0.5, ge=0, description="入场动画时长，单位秒"),
    loop_animation: str = Field(default="", description="循环动画名称"),
    loop_animation_duration: float = Field(default=0.5, ge=0, description="循环动画时长，单位秒"),
    out_animation: str = Field(default="", description="出场动画名称"),
    out_animation_duration: float = Field(default=0.5, ge=0, description="出场动画时长，单位秒"),
    transition: str = Field(default="", description="转场效果名称，如\"溶解\"、\"推近\""),
    transition_duration: float = Field(default=0.5, ge=0, description="转场时长，单位秒")
) -> dict:
    """根据图片URL和时间线生成图片信息"""
    timelines_list = json.loads(timelines_str) if isinstance(timelines_str, str) else timelines_str
    timelines_dict = [{"start": t["start"], "end": t["end"]} for t in timelines_list]
...
@mcp.tool(title="字幕信息", description="根据文本列表和时间线生成字幕信息数组，用于添加到字幕轨道")
def caption_infos(
    texts: str = Field(..., description="""字幕文本JSON字符串数组，格式：["床前明月光","明天我发光"]"""),
    timelines_str: str = Field(..., description="""时间线JSON字符串数组，格式：[{"start":0,"end":284891428},{"start":284891428,"end":579578774}]
- start: number, 开始时间，单位微秒
- end: number, 结束时间，单位微秒"""),
    font_size: int = Field(default=24, ge=1, description="字幕字体大小"),
    keyword_color: str = Field(default="#ff7100", description="关键词颜色，十六进制格式如#ff7100"),
    keyword_font_size: int = Field(default=24, ge=1, description="关键词字体大小"),
    keywords: str = Field(default="", description="关键词列表，多个用|分隔，如\"床前|明天\""),
    in_animation: str = Field(default="", description="入场动画名称，如\"冰雪飘动\"、\"渐显入场\""),
    in_animation_duration: float = Field(default=0.5, ge=0, description="入场动画时长，单位秒"),
    loop_animation: str = Field(default="", description="循环动画名称"),
    loop_animation_duration: float = Field(default=0.5, ge=0, description="循环动画时长，单位秒"),
    out_animation: str = Field(default="", description="出场动画名称"),
    out_animation_duration: float = Field(default=0.5, ge=0, description="出场动画时长，单位秒"),
    transition: str = Field(default="", description="转场效果名称"),
    transition_duration: float = Field(default=0.5, ge=0, description="转场时长，单位秒")
) -> dict:
    """根据文本和时间线生成字幕信息"""
    timelines_list = json.loads(timelines_str) if isinstance(timelines_str, str) else timelines_str
...
def effect_infos(
    effects: str = Field(..., description="""特效名称JSON字符串数组，格式：["金粉闪闪","雪花"]
...
def keyframes_infos(
    ctype: str = Field(..., description="""关键帧动画属性类型，常用值：
- KFTypePositionX: X轴位置
- KFTypePositionY: Y轴位置  
- KFTypeScaleX: X轴缩放
- KFTypeScaleY: Y轴缩放
- KFTypeRotation: 旋转角度
- KFTypeAlpha: 透明度"""),
    offsets: str = Field(..., description="""位置比例JSON字符串数组，格式："0|100"或[0, 100]
- 表示关键帧在片段中的位置比例，0代表开始，100代表结束"""),
    values: str = Field(..., description="""关键帧值JSON字符串数组，格式："1|2"或[1, 2]
- 具体的动画属性值，不同属性类型值范围不同"""),
    segment_infos_str: str = Field(..., description="""片段信息JSON字符串数组，格式：[{"id":"segment_id_1","start":0,"end":5000000}]
- id: string, 片段ID
- start: number, 片段开始时间，单位微秒
- end: number, 片段结束时间，单位微秒"""),
    height: int = Field(default=1080, ge=1, description="视频高度分辨率，单位像素"),
    width: int = Field(default=1920, ge=1, description="视频宽度分辨率，单位像素")
) -> dict:
...
@mcp.tool(title="视频信息", description="根据视频URL列表和时间线生成视频信息数组，用于添加到视频轨道")
def video_infos(
    video_urls: str = Field(..., description="""视频URL JSON字符串数组，格式：["https://example.com/video1.mp4","https://example.com/video2.mp4"]"""),
    timelines_str: str = Field(..., description="""时间线JSON字符串数组，格式：[{"start":0,"end":284891428},{"start":284891428,"end":579578774}]
- start: number, 开始时间，单位微秒
- end: number, 结束时间，单位微秒"""),
    height: int = Field(default=1080, ge=1, description="视频高度，单位像素"),
    width: int = Field(default=1920, ge=1, description="视频宽度，单位像素"),
    mask: str = Field(default="", description="视频蒙版，可选值：圆形、矩形、爱心、星形"),
    transition: str = Field(default="", description="转场效果名称，如\"溶解\"、\"推近\""),
    transition_duration: float = Field(default=1, ge=0, description="转场时长，单位秒"),
    volume: float = Field(default=1.0, ge=0.0, le=10.0, description="音量大小，范围0-10，默认1")
) -> dict:
    """根据视频URL和时间线生成视频信息"""
    timelines_list = json.loads(timelines_str) if isinstance(timelines_str, str) else timelines_str
    timelines_dict = [{"start": t["start"], "end": t["end"]} for t in timelines_list]
...
def search_sticker(
    keyword: str = Field(..., description="搜索关键词，如\"人物\"、\"爱心\"、\"表情\"等")
) -> dict:
    """搜索贴纸"""
...
def get_url(
    output: str = Field(..., description="包含URL链接的文本内容，如\"中国人https://assets.jcaigc.cn/test1.mp3\"")
) -> dict:
    """提取链接"""
...
def str_list_to_objs(
    infos: str = Field(..., description="""URL字符串数组，格式：["https://example.com/file1.mp4","https://example.com/file2.mp4"]""")
) -> dict:
    """字符串列表转化成对象列表"""
...
def str_to_list(
    obj: str = Field(..., description="要转换的JSON字符串，如'{ \"infos\": [\"url1\", \"url2\"] }'")
) -> dict:
    """字符转列表"""
...
def objs_to_str_list(
    outputs: str = Field(..., description="""对象列表JSON字符串数组，格式：[{"output":"https://example.com/file1.mp4"},{"output":"https://example.com/file2.mp4"}]""")
) -> dict:
    """对象列表转化成字符串列表"""
